...
public class Listener {
    ...
    func handleEmitAck (id: Int,
                        error: AnyObject?,
                        data: AnyObject?) {
        guard let ackobject = emitAckListener[id] else { return }
    ...
    func putOnListener(eventName: String,
                       onListener: @escaping (String, AnyObject?) -> Void) {
        self.onListener[eventName] = onListener
    }
    
    func handleOnListener (eventName: String,
                           data: AnyObject?) {
        guard let on = onListener[eventName] else { return }
        on(eventName, data)
    }
    
    func putOnAckListener(eventName: String,
                          onAckListener: @escaping (String, AnyObject?, (AnyObject?, AnyObject?) -> Void ) -> Void) {
        self.onAckListener[eventName] = onAckListener
    }
    
    func handleOnAckListener (eventName: String,
                              data: AnyObject?,
                              ack: (AnyObject?, AnyObject?) -> Void) {
